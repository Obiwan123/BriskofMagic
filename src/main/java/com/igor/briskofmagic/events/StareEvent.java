...
public class StareEvent {
    ...
    public static void onEnderManAngerEvent(EnderManAngerEvent event) {
        LivingEntity wielder = event.getPlayer();
        ResourceKey<Enchantment> key = ModEnchantments.STARE;
        ...
        if (HolderOpt.isPresent()) {
            Holder<Enchantment> stareHolder = HolderOpt.get();
            int enchant = EnchantmentHelper.getEnchantmentLevel(stareHolder, wielder);
            if (enchant > 0) {
